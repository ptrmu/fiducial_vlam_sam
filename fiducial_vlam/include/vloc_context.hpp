...
#define VLOC_ALL_PARAMS \
  ...
  /* Aruco markers */\
  CXT_MACRO_MEMBER(loc_aruco_dictionary_id, int, 0)             /* Aruco dictionary id for localization markers  */ \
...
#define PSL_ALL_PARAMS \
 /* Mssages for vLocnode options */\
  CXT_MACRO_MEMBER(psl_stamp_msgs_with_current_time, int, 0)    /* non-zero => debug mode, helpful for dealing with rviz when playing bags.  */\
  CXT_MACRO_MEMBER(psl_sub_camera_info_best_effort_not_reliable, int, 1) /* subscribe to camera_info message with best_effort (gazebo camera) not reliable (tello_ros) */\
  /* Subscription topics */\
  CXT_MACRO_MEMBER(psl_fiducial_map_sub_topic, std::string, "/fiducial_map")/* topic for subscription to fiducial_vlam_msgs::msg::Map  */\
  CXT_MACRO_MEMBER(psl_camera_info_sub_topic, std::string, "camera_info")   /* topic for subscription to sensor_msgs::msg::CameraInfo associated with the image  */\
  CXT_MACRO_MEMBER(psl_image_raw_sub_topic, std::string, "image_raw")       /* topic for subscription to sensor_msgs::msg::Image */\
  /* Messages to publish */\
  CXT_MACRO_MEMBER(psl_publish_camera_pose, int, 1)             /* non-zero => publish the pose of the camera at every frame  */\
  ...
  CXT_MACRO_MEMBER(psl_publish_tfs_per_marker, int, 0)          /* non-zero => publish the camera tf/pose as determined by each visible marker  */\
  ...
  /* Publish topics */\
  CXT_MACRO_MEMBER(psl_fiducial_observations_pub_topic, std::string, "/fiducial_observations")  /* topic for publishing fiducial observations  */\
  CXT_MACRO_MEMBER(psl_camera_pose_pub_topic, std::string, "camera_pose")    /* topic for publishing camera pose  */\
  CXT_MACRO_MEMBER(psl_base_pose_pub_topic, std::string, "base_pose")        /* topic for publishing base pose  */\
  CXT_MACRO_MEMBER(psl_camera_odometry_pub_topic, std::string, "camera_odom")/* topic for publishing camera odometry  */\
  CXT_MACRO_MEMBER(psl_base_odometry_pub_topic, std::string, "base_odom")    /* topic for publishing base odometry  */\
  CXT_MACRO_MEMBER(psl_image_marked_pub_topic, std::string, "image_marked")  /* topic for republishing the image with axes added to fiducial markers  */\
  /* Frame ids for published messages */\
  CXT_MACRO_MEMBER(psl_map_frame_id, std::string, "map")        /* frame_id for camera pose and tf messages - normally "map"  */\
  CXT_MACRO_MEMBER(psl_camera_frame_id, std::string, "camera")  /* frame_id for the child in the camera tf message  */\
  CXT_MACRO_MEMBER(psl_base_frame_id, std::string, "base_link") /* frame_id for the child in the base_link tf message  */\
  /* End of list */
