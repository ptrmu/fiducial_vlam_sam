...
#define PAMA_PARAM(n, t, d) PAMA_PARAM_CHECK_CMDLINE(n, t, d)
  ...
  private:
    ...
    {
      if (bmm_controller_) {
        auto map = bmm_controller_->build();
        if (map) {
          // Have a new built map. Publish it.
          logger_.info() << map->to_string();
        }
      }
    }
    ...
    void publish_map_and_visualization()
    {
      ...
      header.stamp = now();
      header.frame_id = psm_cxt_.psm_pub_map_frame_id_;
    ...
    tf2_msgs::msg::TFMessage to_tf_message()
    {
      ...
      for (auto &marker_pair: map_->markers()) {
        ...
        std::ostringstream oss_child_frame_id;
        oss_child_frame_id << psm_cxt_.psm_pub_tfs_child_frame_id_ << std::setfill('0') << std::setw(3) << marker.id();
        ...
        msg.header.frame_id = psm_cxt_.psm_pub_map_frame_id_;
    ...
    visualization_msgs::msg::MarkerArray to_marker_array_msg()
    {
      ...
      for (auto &marker_pair: map_->markers()) {
        ...
        marker_msg.header.frame_id = psm_cxt_.psm_pub_map_frame_id_;
